...
MATRIX_DIR="/global/D1/homes/james/sparcity/suitesparse/mtx/gray-v2/"
...
for input_matrix in "${INPUT_MATRICES[@]}"; do
    ...
    gunzip -c "$path_to_input_matrix" > "/work/santiago/tmp2/$base_filename"
    
    # Create directory if it doesn't exist
    mkdir -p "reports"

    # Run the program with the input matrix
    srun ncu -o "reports/report_spmm_1D_gray_$base_filename" -k spmm_1D -c 1 --set full ./row_per_thread "/work/santiago/tmp2/$base_filename"

    # Clean up the temporary decompressed file
    rm "/work/santiago/tmp2/$base_filename"
