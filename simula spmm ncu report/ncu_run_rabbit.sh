...
MATRIX_DIR="/global/D1/homes/santiago/reorderings/rabbit/"
...
for input_matrix in "${INPUT_MATRICES[@]}"; do
    ...
    gunzip -c "$path_to_input_matrix" > "/work/santiago/tmp7/$base_filename"

    # Run the program with the input matrix
    srun ncu -o "reports/report_spmm_32_rabbit_$base_filename" -k load_balancing_kernel -c 1 --set full ./spmm_csr_example  "/work/santiago/tmp7/$base_filename"

    # Clean up the temporary decompressed file
    rm "/work/santiago/tmp7/$base_filename"
