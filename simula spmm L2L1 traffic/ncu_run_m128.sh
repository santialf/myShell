...
MATRIX_DIR="/global/D1/homes/james/sparcity/suitesparse/mtx/metis-a/128-parts/"
...
for input_matrix in "${INPUT_MATRICES[@]}"; do
    ...
    gunzip -c "$path_to_input_matrix" > "/work/santiago/tmp5/$base_filename"

    # Run the program with the input matrix
    srun ncu -k load_balancing_kernel -c 1 --metrics l1tex__m_xbar2l1tex_read_bytes.sum,l1tex__m_l1tex2xbar_write_bytes.sum ./spmm_csr_example "/work/santiago/tmp5/$base_filename"

    # Clean up the temporary decompressed file
    rm "/work/santiago/tmp5/$base_filename"
